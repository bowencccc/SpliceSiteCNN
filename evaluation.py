...
def main():
    ...
    for df in tsv_dfs:
        ...
        thresholds_cnn = [x for x in thresholds_cnn if x < -5]
        precision_list_cnn = []
        recall_list_cnn = []
    ...
    thresholds_markov = [x for x in thresholds_markov if x < 7]
    ...
    plt.title('Precision-Recall Curve for filtering transcripts')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
